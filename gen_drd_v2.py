...
import argparse
# import sys - for defaulting to console printing
...
CATEGORIES = {
    ...
    "Data Glossary": {"Other", "Data Glossary", "Top 20 Data Terms"},
}
...
def load_data(path) -> dict:
    ...
    df = (
        pd.read_csv(path, usecols=["Entity Full Name", "Type", "Entity sub-type", "URL", "Not4DERD",
                                   "Short Description Eng"])  # read in columns from file
        .dropna(subset=["Entity Full Name", "Type"])  # Discard Columns with NA values at Entity full name or Type
    ...
    df["Sub-Type"] = df["Entity sub-type"].str.strip()
    ...
    df.drop("Entity sub-type", axis=1, inplace=True)
...
def main():
    ...
    template = env.get_template("drd.j2")  # Load the Jinja2 template with loader and return it
